...
import getGarageTabsMarine from '@salesforce/apex/OwnGarageController.getGarageTabsMarine';

const CSS_SLDS_IS_ACTIVE = 'slds-is-active';
const CSS_SLDS_SHOW = 'slds-show';
const CSS_SLDS_HIDE = 'slds-hide';

export default class OwnGarageTabsetMarine extends OwnBaseElement {
    @track tabs = [];
    @track tab;

    @wire(getGarageTabsMarine)
    wiredGetGarageTabsMarine({ error, data }) {
        if (data) {
            const tabs = [];
