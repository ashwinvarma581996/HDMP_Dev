...
import getGarageTabsHonda from '@salesforce/apex/OwnGarageController.getGarageTabsHonda';
...
export default class OwnGarageTabsetHonda extends OwnBaseElement {
    @track tabs = [];
    @track tab;
    ...
    @wire(getGarageTabsHonda)
    wiredGetGarageTabsHonda({ error, data }) {
        if (data) {
    ...
    handleActive(event) {
        ...
        if(this.tab === 'Connected Features'){
            window.location = basePath + '/honda-product-connected-features';
            //this.navigate('/honda-product-connected-features', {});
        }else{
